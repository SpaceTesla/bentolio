...
export default function DeliverablesClient() {
  ...
  const current = useMemo(
    () => deliverables[currentIndex] ?? deliverables[0],
    [currentIndex],
  );
  ...
  const structuredData = {
    ...
    itemListElement: deliverables.map((deliverable, index) => ({
      ...
      review: deliverable.client?.testimonial
        ? {
            '@type': 'Review',
            reviewBody: deliverable.client.testimonial,
            author: {
              '@type': 'Person',
              name: deliverable.client.name,
            },
            reviewRating: deliverable.client.rating
              ? {
                  '@type': 'Rating',
                  ratingValue: deliverable.client.rating,
                  bestRating: 5,
                }
              : undefined,
          }
      ...
      <main className="w-full">
        ...
        <div className="grid h-screen grid-cols-1 grid-rows-10 gap-4 overflow-hidden p-4 max-lg:h-auto max-lg:grid-rows-none max-lg:overflow-visible lg:grid-cols-12">
          ...
          <div className="col-span-8 row-span-9 grid grid-cols-subgrid grid-rows-subgrid max-lg:col-span-full max-lg:grid-rows-none max-lg:gap-4 lg:grid-cols-subgrid">
            ...
            {/* Description Section (from data/deliverables) */}
            ...
            {/* Title Section (from data/deliverables) */}
            ...
            {/* Client Section (from data/deliverables) */}
            <div className="relative col-span-4 row-span-3 max-lg:col-span-6 max-lg:h-[20rem] max-md:col-span-full max-md:max-h-[12rem]">
              <div className="bg-primary relative flex h-full overflow-hidden rounded-[20px] p-4 pr-12 pl-6">
                ...
                <div className="hide-scrollbar flex h-full w-full flex-col overflow-y-auto pr-4">
                  <div className="text-2xl font-semibold">
                    {current?.client?.name}
                  </div>
                  <div className="text-xs opacity-80">
                    {current?.client?.designation}
                  </div>
                  ...
                  {typeof current?.client?.rating === 'number' && (
                    <div className="mt-4 flex flex-row items-center gap-1 text-sm opacity-80">
                      <span className="flex flex-row items-center gap-2">
                        {Array.from({ length: 5 }, (_, i) => {
                          const rating = current.client.rating;
                          const starIndex = i + 1;
                          ...
                          if (rating >= starIndex) {
                            ...
                            return (
                              <LuStar
                                key={i}
                                className="text-accent h-6 w-6 fill-current"
                              />
                            );
                            ...
                            // Empty star
                            return (
                              <LuStar key={i} className="text-accent h-6 w-6" />
                            );
                          }
                        })}
                      </span>
            ...
            {/* Tech Stack Section (from data/deliverables) */}
          ...
          <div className="col-span-4 row-span-9 grid grid-cols-subgrid grid-rows-subgrid max-lg:col-span-full max-lg:grid-rows-none max-lg:gap-4 lg:grid-cols-subgrid">
            ...
            <div className="col-span-4 row-span-7 max-lg:col-span-full max-lg:min-h-[18rem]">
              <div className="bg-primary hide-scrollbar relative flex h-full w-full flex-col overflow-y-auto rounded-[20px] p-6">
                <DeliverablesList
                  ...
                  selectedProjectName={deliverables[currentIndex]?.projectName}
                />
              </div>
            </div>
            ...
            <div className="bg-accent relative col-span-4 row-span-1 rounded-[20px] max-lg:col-span-full max-lg:min-h-[5rem]">
              <Link
                ...
                className="flex min-h-full items-center px-6"
              >
                <span className="text-3xl font-semibold">View Projects</span>
                <LuArrowUpRight
                  className={
                    'hover:text-accent absolute right-2 text-4xl transition-all duration-300 max-lg:text-4xl max-md:text-3xl'
                  }
